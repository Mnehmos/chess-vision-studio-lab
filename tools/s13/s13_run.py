...
from s13_common import (AUTHORITY, CVS_ARM_DATASET, EXAM_PROTOCOL_CVS, FAMILY, LAB, OUT, S13,
...
SF_SPECS = {label: TargetSpec(family=FAMILY, authority=AUTHORITY, producer=SF_SHA256,
                              budget={"nodes": budget}, value_path=("scoreCpStm",), pov="stm",
                              k=256.0, lam=1.0) for label, budget in ARMS if label.startswith("SF")}
SF_EXAM_SPEC = TargetSpec(family=FAMILY, authority=AUTHORITY, producer=SF_SHA256,
...
PREREG = S13 / "s13-preregistration.json"
...
def step_exams() -> int:
    ...
    dataset = service.freeze_dataset(
        "N0006", name="s13-exam-sf-deep", record_ids=ids, required_labels=[FAMILY],
        target_spec=SF_EXAM_SPEC, fractions=(0.0, 0.0, 1.0),
        campaign={"purpose": "S13 SF-DEEP exam", "authority": AUTHORITY, "producer": SF_SHA256,
                  "nodes": EXAM_BUDGET, "identities": "the exact D0010 held-out identities"})
    protocol = service.create_eval_protocol(
        name="s13-sf-deep-exam", dataset_id=dataset.id, split="test", k=256.0, lam=1.0,
    ...
    save(exam={"dataset_id": dataset.id, "dataset_manifest_hash": dataset.manifest_hash,
               "protocol_id": protocol.id, "protocol_hash": protocol.protocol_hash,
               "spec_hash": SF_EXAM_SPEC.spec_hash(), "identities": len(ids), "nodes": EXAM_BUDGET})
...
def step_preregister() -> int:
    ...
    prereg = {
        ...
        "teachers": {
            ...
            "SF-1M": {"authority": AUTHORITY, "producer": SF_SHA256, "budget": {"nodes": 1_000_000},
    ...
    digest = write_json(PREREG, prereg)
    (S13 / "s13-preregistration.hash").write_text(digest + "\n", encoding="utf-8")
...
def step_experiment() -> int:
    ...
    for label, budget in ARMS:
        if label == "CVS-4k":
            ...
            spec = SF_SPECS[label]
            dataset = service.freeze_dataset(
                "N0014", name=f"s13-{label.lower()}", record_ids=rows, required_labels=[FAMILY],
                target_spec=spec, fractions=(1.0, 0.0, 0.0),
                campaign={"purpose": "S13 teacher condition", "teacher": label, "authority": AUTHORITY,
    ...
    experiment = service.create_experiment(
        name="S13 teacher authority: CVS vs Stockfish under dual frozen static exams",
        preregistration_hash=(S13 / "s13-preregistration.hash").read_text(encoding="utf-8").strip(),
        ...
        analysis={"design": "three teacher conditions over one population: CVS at 4k nodes, Stockfish "
                  ...
                  "dose": ["SF-1M - SF-32k", "SF-1M - CVS-4k"]},
        experiment_id=xid, notes="3 teacher conditions x 4 widths x 20 seeds = 240 cells; two frozen "
                                 "static exams, never averaged")
    ...
    save(xid=xid, experiment={"id": experiment.id, "arms": len(experiment.arms),
                              "cells": len(expected)},
         conditions={row["label"]: row["record"] for row in conditions})
