...
DUAL = S13 / "s13-dual-exam.json"
RESULT = S13 / "s13-result.json"
SUMMARY = S13 / "s13-result-summary.json"
...
def _exam_splits(service: LabService) -> dict:
    ...
    out = {}
    st = state()
    for name, protocol_id in (("CVS-DEEP", "E0004"), ("SF-DEEP", st["exam"]["protocol_id"])):
...
def step_verify() -> int:
    ...
    recorded = (S13 / "s13-dual-exam.hash").read_text(encoding="utf-8").strip()
    if before != recorded:
        raise LabError("the stored dual-exam artifact no longer matches its recorded hash")
    step_dual()
    after = (S13 / "s13-dual-exam.hash").read_text(encoding="utf-8").strip()
    if after != recorded:
...
def step_report() -> int:
    ...
    result = {
        ...
        "evidence": {"dual_exam": "tools/s13/s13-dual-exam.json",
                     "result": "tools/s13/s13-result.json",
    ...
    summary = {
        ...
        "economics": {label: {"ms_per_label": state()["conditions"][label]["ms_per_label"],
                              "realized_nodes": state()["conditions"][label]["realized_nodes"],
                              "unit_multiples": round(state()["conditions"][label]["unit_multiples"], 2)}
                      for label in LABELS},
