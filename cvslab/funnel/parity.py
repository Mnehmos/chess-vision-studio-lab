"""Fixture parity harness: lab provider vs the frozen legacy funnel (issue #8, plan §8).

Consumes a legacy funnel run directory (`positions.jsonl`, `tier0.jsonl`,
`tier1.jsonl`, as produced by `training/funnel/labeling_funnel.py` — see its
README for the one-line command) and replays the same positions through
`AnalyzeServeProvider`, comparing field-by-field:

* tier0: exact JSON equality of `deterministic_geometry`,
  `bounded_tactical_proof`, `taxonomy`, `uncomputed`, `factsErrors` and
  `factsRegistryVersion` (cost/wall-time excluded);
* tier1: per node-budget exact equality of `scoreCpStm`, `mate`, `bestMove`,
  `pv` and `nodes` (ignore latency; the engine is deterministic at fixed nodes).

Exit code 0 = parity; 1 = mismatches (printed with dotted paths); 2 = usage
error. Any difference that is intentional must be listed in
`INTENTIONAL_DIFFS` with a reason, so the harness never silently accepts drift.

Usage:
    python -m cvslab.funnel.parity --run-dir path/to/legacy/run \
        --engine-root F:/Github/chess-vision-studio-rust-engine [--limit 200]
"""
...
from .providers import AnalyzeServeProvider
from .protocols import Position

# Paths where a difference is accepted, each with its reason. Keep short.
...
def diff_paths(left, right, prefix: str = "") -> list[str]:
    """Dotted paths where two JSON-ish values differ."""
...
def compare_tier0(legacy: dict, lab: dict) -> list[str]:
    ...
    for key in ("status", "deterministic_geometry", "bounded_tactical_proof", "taxonomy",
        ...
        lab_value = lab.get(key)
        # legacy records merge board geometry into deterministic_geometry too
        paths = diff_paths(legacy_value, lab_value, key)
        mismatches += [path for path in paths if path not in INTENTIONAL_DIFFS]
    return mismatches
...
def compare_tier1(legacy: dict, lab_search_by_budget: dict[int, dict]) -> list[str]:
    ...
    for run in legacy.get("search_derived", {}).get("budgets", []):
        ...
        if lab is None:
            ...
            continue
        for key in ("scoreCpStm", "mate", "bestMove", "pv", "nodes"):
            paths = diff_paths(run.get(key), lab.get(key), f"tier1.{budget}.{key}")
            mismatches += [path for path in paths if path not in INTENTIONAL_DIFFS]
    return mismatches
...
def main(argv: list[str] | None = None) -> int:
    ...
    provider = AnalyzeServeProvider(args.engine_root, args=DEFAULT_ANALYZE_ARGS, cwd=args.engine_root)
    failures = 0
    compared = 0
    try:
        for row in positions:
            ...
            if legacy0 is not None:
                batch = provider.label(position)
                if not batch.rows:
                    mismatches = [f"lab.provider.uncomputed={batch.uncomputed}"]
                    lab_record = {}
                else:
                    lab_record = batch.rows[0]
                    mismatches = compare_tier0(legacy0, lab_record)
                compared += 1
                if mismatches:
                    failures += 1
            ...
            if legacy1 is not None:
                ...
                for budget in budgets:
                    if budget and budget <= args.tier1_max_budget:
                        search = provider.search(position, node_budget=budget, pv_plies=args.pv_plies)
    ...
    print(f"parity: compared {compared} records, {failures} with mismatches"
          + (f" (intentional diffs declared: {len(INTENTIONAL_DIFFS)})" if INTENTIONAL_DIFFS else ""))
